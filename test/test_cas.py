...
class TestCas(unittest.TestCase):
    def test_make_hike_chain(self):
        ...
        ret = cas.make_hike_chain(makefile, hike_source_file, hike_dir)
    ...
    def test_make_ebpf_hike_program(self):
        ...
        makefile = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), settings.EXTERNAL_MAKEFILE_PATH)
        hike_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), settings.HIKE_SOURCE_PATH)
        ret = cas.make_ebpf_hike_program(makefile, hike_source_file, hike_dir)
        self.assertEqual(ret, True)
    ...
    def test_hikecc_load(self):
        ...
        ret = cas.load_chain(loader_file)
        self.assertEqual(ret, True)
    ...
    def test_bpftool_prog_loadall(self):
        ...
        ret = cas.bpftool_prog_loadall(bpf_source_file, settings.BPF_FS_PROGS_PATH, type="xdp", pinmaps_map_dir=settings.BPF_FS_MAPS_PATH)
        self.assertEqual(ret, True)
    
    def test_bpftool_prog_load(self):
        # assert: Esiste "xdp_pass.o"
        bpf_source_file = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "runtime/output/build/xdp_pass.o")
        ret = cas.bpftool_prog_load(bpf_source_file, settings.BPF_FS_PROGS_PATH, type="xdp", pinmaps_map_dir=settings.BPF_FS_MAPS_PATH)
        self.assertEqual(ret, True)
    

    def test_bpftool_net_attach(self):
        ret = cas.bpftool_net_attach("xdpdrv", "enp6s0f0", prog="pinned /sys/fs/bpf/progs/xdp_pass")
        self.assertEqual(ret, True)
    
    def test_bpftool_net_attach(self):
        map_name = settings.BPF_FS_MAPS_PATH + "/" + "map_1"
        ret = cas.bpftool_net_attach("pinned " + map_name, key_data="key hex 0b 00 00 00", value="value	pinned /sys/fs/bpf/progs/net/hvxdp_allow_any")
        self.assertEqual(ret, True)
